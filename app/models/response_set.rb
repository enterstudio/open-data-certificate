...
class ResponseSet < ActiveRecord::Base
  ...
  has_one :certification_campaign, through: :certificate_generator
  ...
  def autocomplete(url, automatic=true)
    update_attribute('kitten_data', nil)
    create_kitten_data(url: url, automatic: automatic)
    update_responses(kitten_data.fields)

    name = kitten_data.get(:identifier)
    if name && certification_campaign
      self.documentation_url = certification_campaign.dataset_url_for(name)
    end
  end
